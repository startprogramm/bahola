# Reel 55: Invite Your Director — Bring Your Whole School Into Spring

**Slug:** `reel-55-invite-your-director`
**Publish date:** December 10, 2026 (Thursday — Winter Break & Onboarding week)
**Duration:** 30–40 seconds

---

## Concept

A single teacher's journey from solo user to school-wide champion. They've been using Bahola alone. Now they realize: the director should know about this. The reel walks through the insight and the simple act of sharing — a message, a link, a conversation. Tone: empowering, light, and slightly conspiratorial. "You know something your school doesn't yet. Here's how to share it."

Audience: Bahola teachers who are using the platform but haven't yet brought in leadership. This is the nudge to become an internal advocate.

---

## Hook

**On-screen text (first 2 seconds):**
> **"Siz bilasiz. Direktoringiz bilmaydi."**
> *(You know. Your director doesn't.)*

---

## Shot-by-Shot Storyboard

| # | Timestamp | Shot | Visual | Text Overlay | Audio |
|---|-----------|------|--------|--------------|-------|
| 1 | 0:00–0:04 | Hook | Close-up of a phone screen with Bahola open — grading results visible. Teacher holding the phone, small confident smile. | "Siz bilasiz. Direktoringiz bilmaydi." | Upbeat but subtle — light percussion or minimal beat |
| 2 | 0:04–0:10 | The insight | Teacher looks up from their phone as if realizing something. Casual, natural. Maybe typing starts. | "Bir o'qituvchi emas — butun maktab foydalanishi mumkin." | Music steady |
| 3 | 0:10–0:18 | The action | Screen recording: teacher opens a messaging app, pastes a Bahola link, adds a short message to "Direktor". Sends. Done. | "Direktoringizga yuboring 📩" | Light UI tap sounds with music |
| 4 | 0:18–0:26 | The outcome | Quick cut to a school hallway — other teachers, a classroom, the implication of scale. Montage feel. | "Keyingi semestrda butun maktab birga." | Music builds slightly |
| 5 | 0:26–0:34 | CTA | Clean text card with Bahola logo. | "bahola.uz — Direktoringizga ulashing" | Music fades to close |

---

## Text Overlays (Full Script)

```
Shot 1: "Siz bilasiz. Direktoringiz bilmaydi."
Shot 2: "Bir o'qituvchi emas — butun maktab foydalanishi mumkin."
Shot 3: "Direktoringizga yuboring 📩"
Shot 4: "Keyingi semestrda butun maktab birga."
Shot 5: "bahola.uz — Direktoringizga ulashing"
```

---

## Caption

**Uzbek:**
```
🏫 Siz Baholadan foydalanyapsiz. Direktoringiz hali bilmaydi.

Qishki ta'til — o'zgartirish qilish uchun eng yaxshi vaqt.

Bitta xabar. Bitta havola. Shu kifoya.

"Sinab ko'rish bepul" — bu so'zlar yetarli.

Keyingi semestrda butun maktab birga ishlaydi. 💪

👉 bahola.uz — Direktoringizga ulashing

#Bahola #Direktor #MaktabRahbari #QishkiTatil
#UzbekTaʼlim #EduTech #MaktabHayoti #OqituvchiHayoti
#PresidentSchool #AIGrading #ZamonaviyTaʼlim #OkuvchilarUchun
```

**Russian:**
```
🏫 Вы используете Bahola. Ваш директор ещё не знает.

Зимние каникулы — лучшее время для перемен.

Одно сообщение. Одна ссылка. Этого достаточно.

"Попробовать бесплатно" — этих слов хватит.

В следующем семестре вся школа работает вместе. 💪

👉 bahola.uz — Поделитесь с директором

#Bahola #Директор #ЗимниеКаникулы #ОбразованиеУзбекистана
#EduTech #AIGrading #УчительЖизнь #ШколаБудущего
```

---

## Hashtags

```
#Bahola #Direktor #MaktabRahbari #QishkiTatil
#UzbekTaʼlim #EduTech #MaktabHayoti #OqituvchiHayoti
#PresidentSchool #AIGrading #ZamonaviyTaʼlim #OkuvchilarUchun
```

---

## Music Mood

Light and slightly conspiratorial — like you're sharing a secret with a friend. Minimal electronic beat or a lo-fi track with a bit more energy than reel-54. Still calm, but with a slight forward momentum. The feeling: "I found something good, and I'm about to tell you." No heavy bass. Clean and simple.

---

## Production Notes

- **The hook is the tension.** "Siz bilasiz. Direktoringiz bilmaydi." creates a gap between what the teacher knows and what the school knows. This is the emotional engine of the reel. Don't resolve it too fast — let it sit for a beat.
- **Shot 3 is the key action.** The moment of sending the link to the director is the emotional climax. Make it visible: show the message being typed, the link pasted, the send button tapped. This is what we want viewers to copy.
- **Do NOT show the director's reaction.** We don't have that footage, and faking it looks staged. The outcome is implied by the school hallway shot (Shot 4). Let the audience's imagination fill the gap.
- **Caption tone is peer-level, not preachy.** "Siz bilasiz" (You know) is a compliment — it positions the teacher as the smart one in the room. This should feel like insider knowledge being passed on, not a corporate pitch.
- **Keep it under 38 seconds.** This reel has a clear narrative arc (discovery → action → outcome). Don't pad it. Every second should serve the story.
- **The CTA is a share action, not a signup.** "Direktoringizga ulashing" (Share with your director) — the call-to-action is about advocacy, not self-registration. This reel is targeting existing users who haven't yet recruited their leadership.
- **Bilingual caption.** Uzbek first, Russian second. Both should feel personal and direct.
- **Publish Thursday (Dec 10), not Monday.** Give reel-54 (prep/checklist) space to breathe first. This reel has a different energy and needs its own moment.
