# Reel 65 — Which Assessment Type Should I Use? (Side-by-Side Comparison)

**Campaign:** Week 32 — Feature Deep Dive: Rubric Builder & Assessment Types (Jan 11-15, 2027)
**Duration:** 45-55 seconds
**Theme:** Educational explainer reel — teachers often don't know which assessment format fits their subject. Quick, visual comparison builds confidence to try new formats. Decision-helper content.

---

## Hook Frame (0-3s)
Teacher at whiteboard, marker in hand, looks directly at camera with a slight "help me" expression.

**Text overlay:** "Qaysi baholash turini tanlash kerak?"
*(Translation: "Which assessment type should you choose?")*

---

## Script / Shot List

| Time | Shot | Text Overlay | Voiceover/Caption |
|------|------|-------------|-------------------|
| 0-3s | Teacher looks at camera, slight confused shrug | "Qaysi baholash turini tanlash kerak?" | — |
| 4-7s | Screen splits into 3 panels (left, center, right) | "3 ta variant bor" | "Keling, sodda tushuntiraylik" |
| 8-15s | LEFT panel: Essay icon + handwriting animation. "Ochiq javob" label appears. | **Panel 1: Ochiq javob (Insho / Kengaytirilgan)** | "Fikrlash jarayoni muhim bo'lsa..." |
| 16-20s | LEFT panel: Examples appear one by one: "✏️ Insholar", "📖 Matn tahlili", "🧮 Masala yechimi" | Small checkmarks for each | "...bu turni tanlang" |
| 21-28s | CENTER panel: Numbered list icon. "Tuzilgan javob" label. | **Panel 2: Tuzilgan javob (Aniq qadamlar)** | "Matematik qadamlar yoki faktlar uchun..." |
| 29-33s | CENTER panel: Examples: "🔢 Matematik yechim", "⚗️ Kimyoviy tenglama", "📅 Sanalar va voqealar" | — | "...bu aniqroq" |
| 34-40s | RIGHT panel: Split icon — half essay, half list. "Aralash format" label. | **Panel 3: Aralash format** | "Ikki turdagi savollar aralashsa..." |
| 41-44s | RIGHT panel: Examples: "2 ta qisqa savol + 1 ta kengaytirilgan javob" | "Har qismga alohida mezon" | "ИИ har biri uchun alohida baholaydi" |
| 45-50s | All 3 panels visible. Teacher reappears — now confident, points to center panel. | "Yangi boshlovchilar uchun maslahat:" | "Avval tuzilgan javobdan boshlang — oson va tez" |
| 51-56s | Bahola UI shown — dropdown of assessment types | "Tur tanlash — 1 bosish" | "Keyin rubrik qo'shing — tayyor" |
| 57-60s | End card: Bahola logo + "bahola.uz" | "Birinchi topshiriqni yarating 👇" | — |

---

## Caption (Uzbek)

Qaysi topshiriq turini tanlash kerakligini bilmaysizmi? Biz sizga yordam beramiz. 👇

**Ochiq javob** → Insholar, tahlil, fikrlash
**Tuzilgan javob** → Aniq qadamlar, formulalar, faktlar
**Aralash format** → Bir necha turdagi savollar

Bahola barcha uchta turni qo'llab-quvvatlaydi. Siz turni tanlaysiz — AI shu asosda baholaydi.

Yangi boshlayapsizmi? Tuzilgan javobdan boshlang. U eng aniq natija beradi.

📲 bahola.uz — bepul sinab ko'ring

---

## Caption (Russian)

Не знаете, какой тип задания выбрать? Объясняем просто. 👇

**Открытый ответ** → Эссе, анализ, рассуждение
**Структурированный ответ** → Чёткие шаги, формулы, факты
**Смешанный формат** → Несколько типов вопросов в одном задании

Bahola поддерживает все три формата. Вы выбираете тип — ИИ оценивает по вашим критериям.

Начинаете? Попробуйте структурированный ответ — он даёт самые чёткие результаты.

📲 bahola.uz — попробуйте бесплатно

---

## Hashtags

#Bahola #AssessmentTypes #AIBaholash #OʻqituvchilarUchun #PresidentSchools #Uzbekistan #EdTech #TeacherTools #UzbekEducation #ClassroomTech #RubricBuilder #SmartGrading #TeacherLife #FormativeAssessment

---

## Music Direction
Explanatory, calm but engaging — think explainer video energy. Light background music that doesn't compete with the on-screen text. Slightly upbeat to keep pace with the fast panel reveals. Think: modern educational content, not corporate.

## Production Notes
- The 3-panel split screen is the core visual — must be clean, readable on mobile (vertical format)
- Each panel needs a distinct color/icon to help viewers immediately distinguish the three types
- Use Bahola brand colors for the panels where possible
- The "start with structured answer" tip should be a distinct callout (different color text or box) — it's the key takeaway for new users
- Subtitles in Uzbek throughout (Russian subtitles optional as toggle or alternate caption)
- Consider a "Save this for later" CTA — this reel has reference value teachers will return to
- Fast pace for the first 40s (panels reveal quickly), then slow down for the recommendation and CTA
