# Reel 63 — Before/After: How the First Week Back Used to Feel vs. Now

**Campaign:** Week 31 — First Week Back (Jan 4-8, 2027)
**Duration:** 50-60 seconds
**Theme:** Teacher testimonial concept — before/after emotional arc. How the first week back used to feel vs. now with Bahola. Nostalgic + relief.

---

## Hook Frame (0-3s)
Split screen style. Left: dark, stressed teacher at desk late at night surrounded by paper stacks. Right: same teacher, relaxed, bright environment, phone in hand.

**Text overlay:** "Ilgari va Hozir"
*(Translation: "Before and Now")*

---

## Script / Shot List

| Time | Shot | Text Overlay | Voiceover/Caption |
|------|------|-------------|-------------------|
| 0-3s | Split screen — tired teacher (left, dimly lit) / relaxed teacher (right, bright) | "Ilgari va Hozir" | Soft transition sound |
| 4-10s | BEFORE: Teacher at desk, stacks of notebooks, marking pen in hand. Red-eye look. Clock shows 11 PM. | "Ilgari: Yangi yil birinchi haftasi = tungi baholash" | Slow, heavy music |
| 11-15s | BEFORE: Teacher circles wrong answers, writes comments by hand — laborious | "Har bir ish: 10-15 daqiqa" | Pen scratching sounds |
| 16-20s | BEFORE: Stack of 30 notebooks still untouched. Exhausted sigh. | "30 ta ish... erta tongacha" | — |
| 21-24s | TRANSITION: Screen wipe. Calendar flips to "Yangi Yil 2027" | — | Music shifts — brighter, hopeful |
| 25-30s | NOW: Teacher at bright desk, opens Bahola. Submissions already graded. | "Hozir: AI kechasi ishladi" | "Men uxlayotganda..." |
| 31-36s | NOW: Scroll through 28 graded submissions — each with detailed AI feedback | "28 ta ish ✅ — fikr-mulohaza bilan" | "Men uxlagan paytda AI baholab bo'ldi" |
| 37-42s | NOW: Teacher taps one feedback — rich, specific, in Uzbek. Student name visible. | "Har bir o'quvchiga real fikr-mulohaza" | "Bu faqat siz bilmasangiz bo'ladi..." |
| 43-48s | Teacher looks at camera. Calm smile. Coffee (not red-eyed). Morning light. | "Endi birinchi hafta — oddiy bir hafta" | "Bahola bilan hayot o'zgardi" |
| 49-54s | End card: Bahola logo + "bahola.uz — bepul boshlang" | "Siz ham sinab ko'ring 👇" | — |

---

## Caption (Uzbek)

Yangi yil birinchi haftasi — o'qituvchilar uchun eng og'ir hafta edi.

**Ilgari:**
📚 30 ta daftar
⏰ Kechasi 11 gacha
😔 Qo'lda yozilgan izohlar
😴 Charchoqdan boshlanishi

**Hozir Bahola bilan:**
📱 Barcha ishlar avtomatik baholandi
✅ Batafsil fikr-mulohaza — har bir o'quvchi uchun
☀️ Ertalab faqat natijalarni ko'rish

Siz uxlagan paytda AI ishlaydi.
Birinchi hafta endi — oddiy bir hafta 😊

📲 bahola.uz — bepul boshlang

---

## Caption (Russian)

Первая неделя нового года — самая тяжёлая для учителей.

**Раньше:**
📚 30 тетрадей
⏰ До 11 вечера за столом
😔 Комментарии от руки
😴 Начало года с усталостью

**Сейчас с Bahola:**
📱 Все работы оценены автоматически
✅ Подробная обратная связь — каждому ученику
☀️ Утром — просто просматриваешь результаты

Пока вы спите — ИИ работает.
Первая неделя теперь — просто неделя 😊

📲 bahola.uz — начните бесплатно

---

## Hashtags

#Bahola #IlgariVaHozir #ДоИПосле #BeforeAndAfter #OʻqituvchilarUchun #AIBaholash #Uzbekistan #PresidentSchools #EdTech #BackToSchool #TeacherLife #UzbekEducation #WorkSmarter #TeacherTestimonial

---

## Music Direction
Two distinct moods: heavy, tired, slow for the BEFORE section (think lo-fi but melancholy). Then a clean, uplifting shift for the NOW section — hopeful and warm, morning-energy feel. The contrast in music carries the emotional arc even for silent viewers.

## Production Notes
- This reel lives or dies on the casting and lighting contrast — BEFORE must feel visibly tired and dim, NOW must feel fresh and bright
- Same actor (same teacher) in both sections for credibility — real before/after energy, not just a voiceover
- The "28 submissions graded" stat must show on screen — real UI, not a mockup
- The feedback detail shot (shot 37-42) is critical — viewers need to see that the AI feedback is actually specific and useful, not generic
- Caption lists (before/after) are designed for high shareability — teachers will tag each other
- Subtitle everything — most viewers watch without sound
- Consider adding a "❤️ tag a teacher who needs this" CTA in comments to drive shares
